// TODO: Make `withGqless` decorator work on Next.js `App` component
// TODO: Call `client.dispose()` sometime?
 ...
 */
export const withGqless = ({ssr = false} = {}) => PageComponent => {
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithGqless.getInitialProps = async ctx => {
      ...
      // Prefetch & serialize data if on the server and SSR is enabled
      if (!process.browser && ssr) {
        const {AppTree} = ctx
        const tree = <AppTree pageProps={{...pageProps, gqlessClient}}/>
        try {
